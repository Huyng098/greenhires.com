...
class SSOBase:
    ...
    async def process_login(
        ...
        *,
        ...
        # Fix linkedin oauth 2
        if self.provider == "linkedin":
            body += f"&client_secret={self.client_secret}"

        body = urllib.parse.parse_qs(body)
        auth = httpx.BasicAuth(self.client_id, self.client_secret)
        async with httpx.AsyncClient() as session:
            response = await session.post(token_url, headers=headers, data=body, auth=auth)
            content = response.json()
            self._refresh_token = content.get("refresh_token") or content.get("id_token")
